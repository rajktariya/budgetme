package com.example.budgetme.piechart;
...
public class PieView extends View {
    ...
    private boolean showBoardLine = true;
    ...
    public PieView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.pieView);
        showBoardLine = array.getBoolean(R.styleable.pieView_show_boarder_line, true);
        showPercentLabel = array.getBoolean(R.styleable.pieView_show_percent_text, true);
    ...
    public void setData(ArrayList<PieHelper> helperList) {
        initPies(helperList);
        pieHelperList.clear();
        removeSelectedPie();
    ...
    private void drawText(Canvas canvas, PieHelper pieHelper) {
        ...
        float y = (float) (mViewHeight / 2
                + sth * Math.abs(Math.sin(Math.toRadians(-angel))) * pieRadius / 2);
        canvas.drawText(pieHelper.getPercentStr(), x, y, textPaint);
        canvas.drawText(pieHelper.getTitle(), x, y+60, textPaint);
    }
